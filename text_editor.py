...
def setup_text_options(app, left_panel, comp):
    ...
    (if any) at the current caret position. When a style is applied, a unique tag is created.
    """
    global style_counter  # Make sure our counter is available.
    style_counter = 0
    ...
    def update_options_from_cursor(event=None):
        # Get the index of the current insertion point.
        ...
        if not styled_tags:
            try:
                # Calculate index one character to the left.
                index_before = app.text_editor.index(f"{index} - 1c")
            except tk.TclError:
                index_before = index  # fallback if error occurs
        ...
        if styled_tags:
            ...
            # No styled tag found at the cursor or one character before; revert to defaults.
            font_var.set("Arial")
            size_var.set(12)
            bold_var.set(False)
    ...
    # --- Function to apply style only to the selected text ---
    ...
    tk.OptionMenu(left_panel, font_var, *fonts).pack(fill="x", padx=5)
    ...
    def choose_color():
        ...
        if chosen:
            ...
            update_text_style()  # update style immediately after color change
    tk.Button(left_panel, text="Text Color", command=choose_color).pack(pady=5, padx=5, fill="x")
    
    # Apply Style button applies style only to the currently selected text.
